...
### se defino frec angular digital para el filtro (frec ang dig = frec[Hz]/fm)
            ...
            plt.plot(f,np.abs(fft_evento_sin_medio), color="black", label="Evento sin filtrar")
            plt.plot(f,np.abs(fft_tr_hp), color="violet", label="Evento filtrado con pasa alto de 0.002 Hz")
...
# y todavía queda parte sin ser modificado.
            ...
            '''
            '''
            fft_evento_sin_medio = rfft(traza_evento_sin_medio)
            plt.figure(99)
            plt.plot(traza_evento_sin_medio, color="black", label="Evento sin valor medio")
            plt.plot(tr_taper10, color="violet", label="Evento con taper del 10%")
            plt.legend()
            '''
...
# Taper del 10%
            ...
            tiempo_evento = vec_tiempo(evento)
            
            fig2, (ax1, ax2) = plt.subplots(2)
            fig2.suptitle('Aplicación de Taper tipo Hamming del 10% a evento filtrado',fontsize = 14) # título principal
            ax1.plot(tiempo_evento,traza_evento_sin_medio, color="black", label="Evento sin taper")
            ax1.plot(tiempo_evento,tr_taper10, color="violet", label="Evento con taper del 10%")
            ax1.set_title(inicio_pre) # subtitulo
            ax1.xaxis.label.set_size(20)
            ax1.set_xlabel('tiempo [seg]',fontsize=12)
            ax1.legend()
            ax2.plot(f,np.abs(rfft(traza_evento_sin_medio)), color="black", label="FFT de evento sin taper")
            ax2.plot(f,np.abs(rfft(tr_taper10)), color="violet", label="FFT de evento con taper")
            #ax1.set_xlim(0,1)
            ax2.set_title('Espectro de amplitud')
...
#-----------------------------------------------------------------------------
            ...
            fig8, ax = plt.subplots(4,2)
            fig8.suptitle('Aplicación de filtro Pasa Banda (PB)',fontsize = 14) # título principal               
            ax[0,0].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[0,0].plot(f,np.abs(fft_01), color='violet', label="FFT evento con PB fc=1Hz")
            ax[0,0].legend()
            ax[1,0].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[1,0].plot(f,np.abs(fft_02), color='violet', label="FFT evento con PB fc=1.5Hz")
            ax[1,0].legend()
            ax[2,0].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[2,0].plot(f,np.abs(fft_03), color='violet', label="FFT evento con PB fc=2Hz")
            ax[2,0].legend()
            ax[3,0].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[3,0].plot(f,np.abs(fft_04), color='violet', label="FFT evento con PB fc=2.5Hz")
            ax[3,0].legend()
            ax[0,1].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[0,1].plot(f,np.abs(fft_05), color='violet', label="FFT evento con PB fc=3Hz")
            ax[0,1].legend()
            ax[1,1].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[1,1].plot(f,np.abs(fft_06), color='violet', label="FFT evento con PB fc=3.5Hz")
            ax[1,1].legend()
            ax[2,1].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[2,1].plot(f,np.abs(fft_07), color='violet', label="FFT evento con PB fc=4Hz")
            ax[2,1].legend()
            ax[3,1].plot(f,np.abs(fft_tr_hp), color='black', label='FFT evento filtrado y con taper')
            ax[3,1].plot(f,np.abs(fft_08), color='violet', label="FFT evento con PB fc=4.5Hz")
            ax[3,1].legend()
            for ax in ax.flat:
...
# cálculo del vector tiempo
            ...
            # gráfico
            fig9, ax = plt.subplots(4,2)
            fig9.suptitle(inicio,fontsize = 14) # título principal               
            ax[0,0].plot(t_01,st_dom_time_01[0], color='black')
            ax[0,0].plot(t_01,envolvente_01, color='violet', label="envolvente")
            ax[0,0].set_title('evento con PB fc=1Hz')
            #ax[0,0].legend()
            ax[1,0].plot(t_02,st_dom_time_02[0], color='black')
            ax[1,0].plot(t_02,envolvente_02, color='violet')
            ax[1,0].set_title('evento con PB fc=1.5Hz')            
            #ax[1,0].legend()
            ax[2,0].plot(t_03,st_dom_time_03[0], color='black')
            ax[2,0].plot(t_03,envolvente_03, color='violet')
            ax[2,0].set_title('evento con PB fc=2Hz')            
            #ax[2,0].legend()
            ax[3,0].plot(t_04,st_dom_time_04[0], color='black')
            ax[3,0].plot(t_04,envolvente_04, color='violet')
            ax[3,0].set_title('evento con PB fc=2.5Hz')            
            #ax[3,0].legend()
            ax[0,1].plot(t_05,st_dom_time_05[0], color='black')
            ax[0,1].plot(t_05,envolvente_05, color='violet')
            ax[0,1].set_title('evento con PB fc=3Hz')            
            #ax[0,1].legend()
            ax[1,1].plot(t_06,st_dom_time_06[0], color='black')
            ax[1,1].plot(t_06,envolvente_06, color='violet')
            ax[1,1].set_title('evento con PB fc=3.5Hz')            
            #ax[1,1].legend()
            ax[2,1].plot(t_07,st_dom_time_07[0], color='black')
            ax[2,1].plot(t_07,envolvente_07, color='violet')
            ax[2,1].set_title('evento con PB fc=4Hz')            
            #ax[2,1].legend()
            ax[3,1].plot(t_08,st_dom_time_08[0], color='black')
            ax[3,1].plot(t_08,envolvente_08, color='violet')
            ax[3,1].set_title('evento con PB fc=4.5Hz')            
            #ax[3,1].legend()
            for ax in ax.flat:
            ...
            fig9.legend(lines, labels, loc = 'upper right')
...
# Gráfico de señal y su envolvente
            fig10, ax = plt.subplots(4,2)
            fig10.suptitle(inicio,fontsize = 14) # título principal               
            ax[0,0].plot(t_01,st_dom_time_01[0], color='black')
            ax[0,0].plot(cut_t01,cut_env01, label="envolvente")
            ax[0,0].plot(cut_t01,s_env01, color='violet', label="envolvente suavizada")
            ax[0,0].set_title('evento con PB fc=1Hz')
            ax[1,0].plot(t_02,st_dom_time_02[0], color='black')
            ax[1,0].plot(cut_t02,cut_env02)
            ax[1,0].plot(cut_t02,s_env02, color='violet')
            ax[1,0].set_title('evento con PB fc=1.5Hz')            
            ax[2,0].plot(t_03,st_dom_time_03[0], color='black')
            ax[2,0].plot(cut_t03,cut_env03)
            ax[2,0].plot(cut_t03,s_env03, color='violet')
            ax[2,0].set_title('evento con PB fc=2Hz')            
            ax[3,0].plot(t_04,st_dom_time_04[0], color='black')
            ax[3,0].plot(cut_t04,cut_env04)
            ax[3,0].plot(cut_t04,s_env04, color='violet')
            ax[3,0].set_title('evento con PB fc=2.5Hz')            
            ax[0,1].plot(t_05,st_dom_time_05[0], color='black')
            ax[0,1].plot(cut_t05,cut_env05)
            ax[0,1].plot(cut_t05,s_env05, color='violet')
            ax[0,1].set_title('evento con PB fc=3Hz')            
            ax[1,1].plot(t_06,st_dom_time_06[0], color='black')
            ax[1,1].plot(cut_t06,cut_env06)
            ax[1,1].plot(cut_t06,s_env06, color='violet')
            ax[1,1].set_title('evento con PB fc=3.5Hz')            
            ax[2,1].plot(t_07,st_dom_time_07[0], color='black')
            ax[2,1].plot(cut_t07,cut_env07)
            ax[2,1].plot(cut_t07,s_env07, color='violet')
            ax[2,1].set_title('evento con PB fc=4Hz')            
            ax[3,1].plot(t_08,st_dom_time_08[0], color='black')
            ax[3,1].plot(cut_t08,cut_env08)
            ax[3,1].plot(cut_t08,s_env08, color='violet')
            ax[3,1].set_title('evento con PB fc=4.5Hz')            
            for ax in ax.flat:
                ax.set(xlabel='tiempo [seg]')
            for ax in  fig10.get_axes():
                ax.label_outer()                       
            lines = []
            labels = []
            for ax in fig10.axes:
                ...
                lines.extend(axLine)
                labels.extend(axLabel)
            fig10.legend(lines, labels, loc = 'upper right')
            fig10.tight_layout()
'''
...
# valores por donde comenzar a ajustar.
            ...
            fig11, ax = plt.subplots(nrows=4,ncols=2)
            plt.subplots_adjust(left=0.125, bottom=0.11, right=0.9, top=0.88, wspace=0.114, hspace=0.545)
            fig11.suptitle("Ajuste lineal a coda corregida por DG",fontsize = 14) # título principal               
            ax[0,0].plot(lpt01_grow,cut_prediccion01, label="Alpt = {:.4f}t + {:.4f}".format(p1_01,p2_01))
            ax[0,0].plot(lpt01_grow,cut_logAlpt01)
            ax[0,0].legend(loc = 'best')
            ax[0,0].set_title('fc=1Hz')
            ax[1,0].plot(lpt02_grow,cut_prediccion02, label="Alpt = {:.4f}t + {:.4f}".format(p1_02,p2_02))
            ax[1,0].plot(lpt02_grow,cut_logAlpt02)
            ax[1,0].legend(loc = 'best')
            ax[1,0].set_title('fc=1.5Hz')            
...
#-----------------------------------------------------------------------------
            ...
            if p1_01 < 0:
                ...
                lapsetime01_inicio = lapsetime01[:len_lpt01]
                env_01_inicio = envolvente_01[:len_lpt01]
                Alpt01_inicio = env_01_inicio * lapsetime01_inicio
                
                '''lapsetime01_completo = t_evento + delta_max01 + t_01
                Alpt01_completo = envolvente_01 * lapsetime01_completo'''
                
                popt_01_d, pcov_01_d = curve_fit(model_dif, lapsetime01_inicio, np.log(Alpt01_inicio))
            ...
            if p1_02 < 0:
                lapsetime02_inicio = lapsetime02[:len_lpt02]
                env_02_inicio = envolvente_02[:len_lpt02]
                Alpt02_inicio = env_02_inicio * lapsetime02_inicio
                
                popt_02_d, pcov_02_d = curve_fit(model_dif, lapsetime02_inicio, np.log(Alpt02_inicio))
            ...
            if p1_03 < 0:
                lapsetime03_inicio = lapsetime03[:len_lpt03]
                env_03_inicio = envolvente_03[:len_lpt03]
                Alpt03_inicio = env_03_inicio * lapsetime03_inicio
                
                popt_03_d, pcov_03_d = curve_fit(model_dif, lapsetime03_inicio, np.log(Alpt03_inicio))
            ...
            if p1_04 < 0:
                lapsetime04_inicio = lapsetime04[:len_lpt04]
                env_04_inicio = envolvente_04[:len_lpt04]
                Alpt04_inicio = env_04_inicio * lapsetime04_inicio
                
                popt_04_d, pcov_04_d = curve_fit(model_dif, lapsetime04_inicio, np.log(Alpt04_inicio))
            ...
            if p1_05 < 0:
                lapsetime05_inicio = lapsetime05[:len_lpt05]
                env_05_inicio = envolvente_05[:len_lpt05]
                Alpt05_inicio = env_05_inicio * lapsetime05_inicio
                
                popt_05_d, pcov_05_d = curve_fit(model_dif, lapsetime05_inicio, np.log(Alpt05_inicio))
            ...
            if p1_06 < 0:
                lapsetime06_inicio = lapsetime06[:len_lpt06]
                env_06_inicio = envolvente_06[:len_lpt06]
                Alpt06_inicio = env_06_inicio * lapsetime06_inicio
                
                popt_06_d, pcov_06_d = curve_fit(model_dif, lapsetime06_inicio, np.log(Alpt06_inicio))
            ...
            if p1_07 < 0:
                lapsetime07_inicio = lapsetime07[:len_lpt07]
                env_07_inicio = envolvente_07[:len_lpt07]
                Alpt07_inicio = env_07_inicio * lapsetime07_inicio
                
                popt_07_d, pcov_07_d = curve_fit(model_dif, lapsetime07_inicio, np.log(Alpt07_inicio))
            ...
            if p1_08 < 0:
                lapsetime08_inicio = lapsetime08[:len_lpt08]
                env_08_inicio = envolvente_08[:len_lpt08]
                Alpt08_inicio = env_08_inicio * lapsetime08_inicio
                
                popt_08_d, pcov_08_d = curve_fit(model_dif, lapsetime08_inicio, np.log(Alpt08_inicio))
...
#%%
    # GUARDO LAS VARIABLES!!!! (FRECUENCIAS, Q, TODO)
