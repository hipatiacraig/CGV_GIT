...
# valores por donde comenzar a ajustar.
            ...
            # pide el paquete de regresión lineal
                ...
                lapsetime01_reshape = lapsetime01.reshape(-1,1)
                model01 = LinearRegression().fit(lapsetime01_reshape, logAlpt01)
                prediccion01 = model01.predict(lapsetime01_reshape)
                r_sq01 = model01.score(lapsetime01_reshape, logAlpt01)
                #print("Resultados del ajuste lineal. Frecuencias entre 0.5 - 1.5 Hz")
                #print(f"coefficient of determination: {r_sq01}")
                p2_01 = model01.intercept_
                ...
                if p1_01[0] < 0:
                    print("pendiente: {:.4f}. len(lapsetime01)={:}".format(p1_01[0],len(lapsetime01)))
                    print("intersección: {:.4f}".format(p2_01))
                    lapsetime01_new = lapsetime01
                    break
                
                elif p1_01[0] > 0:
                    #print("pendiente: {:.4f}. len(lapsetime01)={:}".format(p1_01[0],len(lapsetime01)))
                    lapsetime01_new = lapsetime01[:len(lapsetime01)-250]
                    cut_env01 = cut_env01[:len(lapsetime01_new)]
                    
            if p1_01[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion01 = prediccion01[:len(lapsetime01_new)]
            cut_Alpt01 = Alpt01[:len(lapsetime01_new)]
            cut_cut_env01 = cut_env01[:len(lapsetime01_new)]
            cut_logAlpt01 = logAlpt01[:len(lapsetime01_new)]
...
# el ajuste lineal
            ...
            if p1_01[0] < 0:
                ...
                p0_01 = (np.exp(p2_01),p1_01[0]) # hago p1[0] xq es un array y yo necesito el elemento
                # ajuste
                popt_01, pcov_01 = curve_fit(model_exp, lapsetime01_new, cut_Alpt01, p0_01)
...
### Difusión
            ...
            if p1_01[0] < 0:
                ...
                cortar_lpt_final_01 = len(lapsetime01_new) + len_diferencia_lpt_01 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f01,'lapsetime':len(lapsetime01_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_01),'b_lin':-p1_01,'a_exp':popt_01[0],'b_exp':popt_01[1],'Qc_SBS': Q_01,'a1_dif':popt_01_d[0],'a2_dif':popt_01_d[1],'a3_dif':popt_01_d[2],'Qi':Qi_01,'d':d_01,'Qs':Qs_01}
...
# valores por donde comenzar a ajustar.
            ...
            while len(lapsetime02) > 30*50:
                Alpt02 = cut_env02 * lapsetime02
                logAlpt02 = np.log(Alpt02).reshape(-1)
                lapsetime02_reshape = lapsetime02.reshape(-1,1)
                model02 = LinearRegression().fit(lapsetime02_reshape, logAlpt02)
                prediccion02 = model02.predict(lapsetime02_reshape)
                r_sq02 = model02.score(lapsetime02_reshape, logAlpt02)
                p2_02 = model02.intercept_
                p1_02 = model02.coef_

                if p1_02[0] < 0:
                    lapsetime02_new = lapsetime02
                    print("pendiente: {:.4f}. len(lapsetime02)={:}".format(p1_02[0],len(lapsetime02_new)))
                    print("intersección: {:.4f}".format(p2_02))
                    break
                
                elif p1_02[0] > 0:
                    lapsetime02_new = lapsetime02[:len(lapsetime02)-250]
                    cut_env02 = cut_env02[:len(lapsetime02_new)]
                    #print("pendiente: {:.4f}. len(lapsetime02_new)={:}".format(p1_02[0],len(lapsetime01_new)))

                    
            if p1_02[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion02 = prediccion02[:len(lapsetime02_new)]
            cut_Alpt02 = Alpt02[:len(lapsetime02_new)]
            cut_cut_env02 = cut_env02[:len(lapsetime02_new)]
            cut_logAlpt02 = logAlpt02[:len(lapsetime02_new)]
...
# el ajuste lineal
            ...
            if p1_02[0] < 0:
                p0_02 = (np.exp(p2_02),p1_02[0])
                popt_02, pcov_02 = curve_fit(model_exp, lapsetime02_new, cut_Alpt02, p0_02)
...
### Difusión
            ...
            if p1_02[0] < 0:
                ...
                cortar_lpt_final_02 = len(lapsetime02_new) + len_diferencia_lpt_02 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            dict = {'UTCDateTime':inicio,'Fc':f02,'lapsetime':len(lapsetime02_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_02),'b_lin':-p1_02,'a_exp':popt_02[0],'b_exp':popt_02[1],'Qc_SBS': Q_02,'a1_dif':popt_02_d[0],'a2_dif':popt_02_d[1],'a3_dif':popt_02_d[2],'Qi':Qi_02,'d':d_02,'Qs':Qs_02}
...
###### CÁLCULOS PRIORIZANDO UNA CODA MÁS LARGA:
#------------------------------------------------            
### Primer ajuste: lineal

            print("Resultados del ajuste lineal. Frecuencias entre 1.5 - 2.5 Hz")
            
            while len(lapsetime03) > 30*50:
                Alpt03 = cut_env03 * lapsetime03
                logAlpt03 = np.log(Alpt03).reshape(-1)
                lapsetime03_reshape = lapsetime03.reshape(-1,1)
                model03 = LinearRegression().fit(lapsetime03_reshape, logAlpt03)
                prediccion03 = model03.predict(lapsetime03_reshape)
                r_sq03 = model03.score(lapsetime03_reshape, logAlpt03)
                p2_03 = model03.intercept_
                p1_03 = model03.coef_

                if p1_03[0] < 0:
                    lapsetime03_new = lapsetime03
                    print("pendiente: {:.4f}. len(lapsetime03)={:}".format(p1_03[0],len(lapsetime03_new)))
                    print("intersección: {:.4f}".format(p2_03))
                    break
                
                elif p1_03[0] > 0:
                    lapsetime03_new = lapsetime03[:len(lapsetime03)-250]
                    cut_env03 = cut_env03[:len(lapsetime03_new)]
                    #print("pendiente: {:.4f}. len(lapsetime03_new)={:}".format(p1_03[0],len(lapsetime03_new)))

                    
            if p1_03[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion03 = prediccion03[:len(lapsetime03_new)]
            cut_Alpt03 = Alpt03[:len(lapsetime03_new)]
            cut_cut_env03 = cut_env03[:len(lapsetime03_new)]
            cut_logAlpt03 = logAlpt03[:len(lapsetime03_new)]
...
# el ajuste lineal
            ...
            if p1_03[0] < 0:
                p0_03 = (np.exp(p2_03),p1_03[0])
                popt_03, pcov_03 = curve_fit(model_exp, lapsetime03_new, cut_Alpt03, p0_03)
...
### Difusión
            ...
            if p1_03[0] < 0:
                lapsetime03_entero = t_evento + delta_max03 + t_03 # lapsetime desde el inicio del tag
                len_diferencia_lpt_03 = len(lapsetime03_entero)-len(lapsetime03) # veo cuántas muestas hay entre el lapsetime desde max amplt y el lapsetime desde el inicio
                cortar_lpt_final_03 = len(lapsetime03_new) + len_diferencia_lpt_03 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lpt=lapsetime01[-1:] # asi en cada lapse time, nos quedamos con el ultimo elemento como float para imprimirlo
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f03,'lapsetime':len(lapsetime03_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_03),'b_lin':-p1_03,'a_exp':popt_03[0],'b_exp':popt_03[1],'Qc_SBS': Q_03,'a1_dif':popt_03_d[0],'a2_dif':popt_03_d[1],'a3_dif':popt_03_d[2],'Qi':Qi_03,'d':d_03,'Qs':Qs_03}
...
### Primer ajuste: lineal
            ...
            while len(lapsetime04) > 30*50:
                Alpt04 = cut_env04 * lapsetime04
                logAlpt04 = np.log(Alpt04).reshape(-1)
                lapsetime04_reshape = lapsetime04.reshape(-1,1)
                model04 = LinearRegression().fit(lapsetime04_reshape, logAlpt04)
                prediccion04 = model04.predict(lapsetime04_reshape)
                r_sq04 = model04.score(lapsetime04_reshape, logAlpt04)
                p2_04 = model04.intercept_
                p1_04 = model04.coef_

                if p1_04[0] < 0:
                    lapsetime04_new = lapsetime04
                    print("pendiente: {:.4f}. len(lapsetime04)={:}".format(p1_04[0],len(lapsetime04_new)))
                    print("intersección: {:.4f}".format(p2_04))
                    break
                
                elif p1_04[0] > 0:
                    lapsetime04_new = lapsetime04[:len(lapsetime04)-250]
                    cut_env04 = cut_env04[:len(lapsetime04_new)]
                    #print("pendiente: {:.4f}. len(lapsetime04_new)={:}".format(p1_04[0],len(lapsetime04_new)))

                    
            if p1_04[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion04 = prediccion04[:len(lapsetime04_new)]
            cut_Alpt04 = Alpt04[:len(lapsetime04_new)]
            cut_cut_env04 = cut_env04[:len(lapsetime04_new)]
            cut_logAlpt04 = logAlpt04[:len(lapsetime04_new)]
...
### Segundo ajuste: exponencial y cálculo del Q
            ...
            if p1_04[0] < 0:
                p0_04 = (np.exp(p2_04),p1_04[0])
                popt_04, pcov_04 = curve_fit(model_exp, lapsetime04_new, cut_Alpt04, p0_04)
...
### Difusión
            ...
            if p1_04[0] < 0:
                lapsetime04_entero = t_evento + delta_max04 + t_04 # lapsetime desde el inicio del tag
                len_diferencia_lpt_04 = len(lapsetime04_entero)-len(lapsetime04) # veo cuántas muestas hay entre el lapsetime desde max amplt y el lapsetime desde el inicio
                cortar_lpt_final_04 = len(lapsetime04_new) + len_diferencia_lpt_04 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lpt=lapsetime01[-1:] # asi en cada lapse time, nos quedamos con el ultimo elemento como float para imprimirlo
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f04,'lapsetime':len(lapsetime04_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_04),'b_lin':-p1_04,'a_exp':popt_04[0],'b_exp':popt_04[1],'Qc_SBS': Q_04,'a1_dif':popt_04_d[0],'a2_dif':popt_04_d[1],'a3_dif':popt_04_d[2],'Qi':Qi_04,'d':d_04,'Qs':Qs_04}
...
### Primer ajuste: lineal
            ...
            while len(lapsetime05) > 30*50:
                Alpt05 = cut_env05 * lapsetime05
                logAlpt05 = np.log(Alpt05).reshape(-1)
                lapsetime05_reshape = lapsetime05.reshape(-1,1)
                model05 = LinearRegression().fit(lapsetime05_reshape, logAlpt05)
                prediccion05 = model05.predict(lapsetime05_reshape)
                r_sq05 = model05.score(lapsetime05_reshape, logAlpt05)
                p2_05 = model05.intercept_
                p1_05 = model05.coef_

                if p1_05[0] < 0:
                    lapsetime05_new = lapsetime05
                    print("pendiente: {:.4f}. len(lapsetime05)={:}".format(p1_05[0],len(lapsetime05_new)))
                    print("intersección: {:.4f}".format(p2_05))
                    break
                
                elif p1_05[0] > 0:
                    lapsetime05_new = lapsetime05[:len(lapsetime05)-250]
                    cut_env05 = cut_env05[:len(lapsetime05_new)]
                    #print("pendiente: {:.4f}. len(lapsetime05_new)={:}".format(p1_05[0],len(lapsetime05_new)))

                    
            if p1_05[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion05 = prediccion05[:len(lapsetime05_new)]
            cut_Alpt05 = Alpt05[:len(lapsetime05_new)]
            cut_cut_env05 = cut_env05[:len(lapsetime05_new)]
            cut_logAlpt05 = logAlpt05[:len(lapsetime05_new)]
...
### Segundo ajuste: exponencial y cálculo del Q
            ...
            if p1_05[0] < 0:
                p0_05 = (np.exp(p2_05),p1_05[0])
                popt_05, pcov_05 = curve_fit(model_exp, lapsetime05_new, cut_Alpt05, p0_05)
...
### Difusión
            ...
            if p1_05 < 0:
                lapsetime05_entero = t_evento + delta_max05 + t_05 # lapsetime desde el inicio del tag
                len_diferencia_lpt_05 = len(lapsetime05_entero)-len(lapsetime05) # veo cuántas muestas hay entre el lapsetime desde max amplt y el lapsetime desde el inicio
                cortar_lpt_final_05 = len(lapsetime05_new) + len_diferencia_lpt_05 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lpt=lapsetime01[-1:] # asi en cada lapse time, nos quedamos con el ultimo elemento como float para imprimirlo
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f05,'lapsetime':len(lapsetime05_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_05),'b_lin':-p1_05,'a_exp':popt_05[0],'b_exp':popt_05[1],'Qc_SBS': Q_05,'a1_dif':popt_05_d[0],'a2_dif':popt_05_d[1],'a3_dif':popt_05_d[2],'Qi':Qi_05,'d':d_05,'Qs':Qs_05}
...
### Primer ajuste: lineal
            ...
            while len(lapsetime06) > 30*50:
                Alpt06 = cut_env06 * lapsetime06
                logAlpt06 = np.log(Alpt06).reshape(-1)
                lapsetime06_reshape = lapsetime06.reshape(-1,1)
                model06 = LinearRegression().fit(lapsetime06_reshape, logAlpt06)
                prediccion06 = model06.predict(lapsetime06_reshape)
                r_sq06 = model06.score(lapsetime06_reshape, logAlpt06)
                p2_06 = model06.intercept_
                p1_06 = model06.coef_

                if p1_06[0] < 0:
                    lapsetime06_new = lapsetime06
                    print("pendiente: {:.4f}. len(lapsetime06)={:}".format(p1_06[0],len(lapsetime06_new)))
                    print("intersección: {:.4f}".format(p2_06))
                    break
                
                elif p1_06[0] > 0:
                    lapsetime06_new = lapsetime06[:len(lapsetime06)-250]
                    cut_env06 = cut_env06[:len(lapsetime06_new)]
                    #print("pendiente: {:.4f}. len(lapsetime06_new)={:}".format(p1_06[0],len(lapsetime06_new)))

                    
            if p1_06[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion06 = prediccion06[:len(lapsetime06_new)]
            cut_Alpt06 = Alpt06[:len(lapsetime06_new)]
            cut_cut_env06 = cut_env06[:len(lapsetime06_new)]
            cut_logAlpt06 = logAlpt06[:len(lapsetime06_new)]
...
### Segundo ajuste: exponencial y cálculo del Q
            ...
            if p1_06[0] < 0:
                p0_06 = (np.exp(p2_06),p1_06[0])
                popt_06, pcov_06 = curve_fit(model_exp, lapsetime06_new, cut_Alpt06, p0_06)
...
### Difusión
            ...
            if p1_06[0] < 0:
                lapsetime06_entero = t_evento + delta_max06 + t_06 # lapsetime desde el inicio del tag
                len_diferencia_lpt_06 = len(lapsetime06_entero)-len(lapsetime06) # veo cuántas muestas hay entre el lapsetime desde max amplt y el lapsetime desde el inicio
                cortar_lpt_final_06= len(lapsetime06_new) + len_diferencia_lpt_06 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lpt=lapsetime01[-1:] # asi en cada lapse time, nos quedamos con el ultimo elemento como float para imprimirlo
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f06,'lapsetime':len(lapsetime06_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_06),'b_lin':-p1_06,'a_exp':popt_06[0],'b_exp':popt_06[1],'Qc_SBS': Q_06,'a1_dif':popt_06_d[0],'a2_dif':popt_06_d[1],'a3_dif':popt_06_d[2],'Qi':Qi_06,'d':d_06,'Qs':Qs_06}
...
### Primer ajuste: lineal
            ...
            while len(lapsetime07) > 30*50:
                Alpt07 = cut_env07 * lapsetime07
                logAlpt07 = np.log(Alpt07).reshape(-1)
                lapsetime07_reshape = lapsetime07.reshape(-1,1)
                model07 = LinearRegression().fit(lapsetime07_reshape, logAlpt07)
                prediccion07 = model07.predict(lapsetime07_reshape)
                r_sq07 = model07.score(lapsetime07_reshape, logAlpt07)
                p2_07 = model07.intercept_
                p1_07 = model07.coef_

                if p1_07[0] < 0:
                    lapsetime07_new = lapsetime07
                    print("pendiente: {:.4f}. len(lapsetime07)={:}".format(p1_07[0],len(lapsetime07_new)))
                    print("intersección: {:.4f}".format(p2_07))
                    break
                
                elif p1_07[0] > 0:
                    lapsetime07_new = lapsetime07[:len(lapsetime07)-250]
                    cut_env07 = cut_env07[:len(lapsetime07_new)]
                    #print("pendiente: {:.4f}. len(lapsetime07_new)={:}".format(p1_07[0],len(lapsetime07_new)))

                    
            if p1_07[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion07 = prediccion07[:len(lapsetime07_new)]
            cut_Alpt07 = Alpt07[:len(lapsetime07_new)]
            cut_cut_env07 = cut_env07[:len(lapsetime07_new)]
            cut_logAlpt07 = logAlpt07[:len(lapsetime07_new)]
...
### Segundo ajuste: exponencial y cálculo del Q
            ...
            if p1_07[0] < 0:
                p0_07 = (np.exp(p2_07),p1_07[0])
                popt_07, pcov_07 = curve_fit(model_exp, lapsetime07_new, cut_Alpt07, p0_07)
...
### Difusión
            ...
            if p1_07[0] < 0:
                lapsetime07_entero = t_evento + delta_max07 + t_07 # lapsetime desde el inicio del tag
                len_diferencia_lpt_07 = len(lapsetime07_entero)-len(lapsetime07) # veo cuántas muestas hay entre el lapsetime desde max amplt y el lapsetime desde el inicio
                cortar_lpt_final_07 = len(lapsetime07_new) + len_diferencia_lpt_07 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lpt=lapsetime01[-1:] # asi en cada lapse time, nos quedamos con el ultimo elemento como float para imprimirlo
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f07,'lapsetime':len(lapsetime07_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_07),'b_lin':-p1_07,'a_exp':popt_07[0],'b_exp':popt_07[1],'Qc_SBS': Q_07,'a1_dif':popt_07_d[0],'a2_dif':popt_07_d[1],'a3_dif':popt_07_d[2],'Qi':Qi_07,'d':d_07,'Qs':Qs_07}
...
### Primer ajuste: lineal
            ...
            while len(lapsetime08) > 30*50:
                Alpt08 = cut_env08 * lapsetime08
                logAlpt08 = np.log(Alpt08).reshape(-1)
                lapsetime08_reshape = lapsetime08.reshape(-1,1)
                model08 = LinearRegression().fit(lapsetime08_reshape, logAlpt08)
                prediccion08 = model08.predict(lapsetime08_reshape)
                r_sq08 = model08.score(lapsetime08_reshape, logAlpt08)
                p2_08 = model08.intercept_
                p1_08 = model08.coef_

                if p1_08[0] < 0:
                    lapsetime08_new = lapsetime08
                    print("pendiente: {:.4f}. len(lapsetime08)={:}".format(p1_08[0],len(lapsetime08_new)))
                    print("intersección: {:.4f}".format(p2_08))
                    break
                
                elif p1_08[0] > 0:
                    lapsetime08_new = lapsetime08[:len(lapsetime08)-250]
                    cut_env08 = cut_env08[:len(lapsetime08_new)]
                    #print("pendiente: {:.4f}. len(lapsetime08_new)={:}".format(p1_08[0],len(lapsetime08_new)))

                    
            if p1_08[0] > 0:
                print("No hay pendiente negativa")
            
            cut_prediccion08 = prediccion08[:len(lapsetime08_new)]
            cut_Alpt08 = Alpt08[:len(lapsetime08_new)]
            cut_cut_env08 = cut_env08[:len(lapsetime08_new)]
            cut_logAlpt08 = logAlpt08[:len(lapsetime08_new)]            
...
### Segundo ajuste: exponencial y cálculo del Q
            ...
            if p1_08[0] < 0:
                p0_08 = (np.exp(p2_08),p1_08[0])
                popt_08, pcov_08 = curve_fit(model_exp, lapsetime08_new, cut_Alpt08, p0_08)
...
### Difusión
            ...
            if p1_08[0] < 0:
                lapsetime08_entero = t_evento + delta_max08 + t_08 # lapsetime desde el inicio del tag
                len_diferencia_lpt_08 = len(lapsetime08_entero)-len(lapsetime08) # veo cuántas muestas hay entre el lapsetime desde max amplt y el lapsetime desde el inicio
                cortar_lpt_final_08 = len(lapsetime08_new) + len_diferencia_lpt_08 # porque len_lpt esta considerado desde amplt max hasta el mejor ajuste
...
### Grabado de datos
            ...
            #lpt=lapsetime01[-1:] # asi en cada lapse time, nos quedamos con el ultimo elemento como float para imprimirlo
            #lptfin=lpt[0]
            dict = {'UTCDateTime':inicio,'Fc':f08,'lapsetime':len(lapsetime08_new),'dist':dist,'p_dg':p,'vel':vel,'a_lin':np.exp(p2_08),'b_lin':-p1_08,'a_exp':popt_08[0],'b_exp':popt_08[1],'Qc_SBS': Q_08,'a1_dif':popt_08_d[0],'a2_dif':popt_08_d[1],'a3_dif':popt_08_d[2],'Qi':Qi_08,'d':d_08,'Qs':Qs_08}
...
# envolvente y ajuste qc
            ...
            if p1_01 < 0:
                ...
                plt.plot(lapsetime01_new,model_exp(lapsetime01_new,popt_01[0],popt_01[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_02 < 0:
                ...
                plt.plot(lapsetime02_new,model_exp(lapsetime02_new,popt_02[0],popt_02[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_03 < 0:
                ...
                plt.plot(lapsetime03_new,model_exp(lapsetime03_new,popt_03[0],popt_03[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_04 < 0:
                ...
                plt.plot(lapsetime04_new,model_exp(lapsetime04_new,popt_04[0],popt_04[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_05 < 0:
                ...
                plt.plot(lapsetime05_new,model_exp(lapsetime05_new,popt_05[0],popt_05[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_06 < 0:
                ...
                plt.plot(lapsetime06_new,model_exp(lapsetime06_new,popt_06[0],popt_06[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_07 < 0:
                ...
                plt.plot(lapsetime07_new,model_exp(lapsetime07_new,popt_07[0],popt_07[1]),color='violet',label="ajuste no lineal")
            ...
            if p1_08 < 0:
                ...
                plt.plot(lapsetime08_new,model_exp(lapsetime08_new,popt_08[0],popt_08[1]),color='violet',label="ajuste no lineal")
