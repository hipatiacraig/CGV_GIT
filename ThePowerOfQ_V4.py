...
# valores por donde comenzar a ajustar.
            ...
            intersec01 = model01.intercept_
            print(f"intersección: {intersec01}")
            pendiente01 = model01.coef_
            print(f"pendiente: {pendiente01}")
...
# PRUEBA 1:
            # se define la función que se quiere ajustar
            def func(x, a, b):
                return a * np.exp(-b * x)
            '''
            # se dan valores iniciales para a y b para "guiar" al ajuste (a,b)
            #val_inicial = (np.exp(intersec01),pendiente01)
            val_inicial = (np.exp(intersec01),pendiente01)
            popt, pcov = curve_fit(func, lapsetime01, pAlpt01,val_inicial)
            '''


# PRUEBA 2:
            popt, pcov = curve_fit(func, cut_env01, lapsetime01)
            
            
            p = np.polyfit(lapsetime01, np.log(cut_env01), 1)
            a = np.exp(p[1])
            b = p[0]

            x_fitted = np.linspace(np.min(lapsetime01), np.max(lapsetime01), 100)
            y_fitted = a * np.exp(-b * x_fitted)
            
            ax = plt.axes()
            ax.scatter(lapsetime01, cut_env01, label='Raw data')
...
# PRUEBA 3:           
            popt, pcov = curve_fit(lambda t, a, b, c: a * np.exp(b * t) + c, lapsetime01, cut_env01)
            a = popt[0]
            b = popt[1]
            c = popt[2]                        
            x_fitted = np.linspace(np.min(lapsetime01), np.max(lapsetime01), 100)
            ...
            # Plot
            ax = plt.axes()
            ax.plot(lapsetime01, cut_env01, label='Raw data')
            ax.plot(x_fitted, y_fitted, 'k', label='Fitted curve')
...
# PRUEBA 4:            
            ...
            def fit_exp_nonlinear(t, y):
                opt_parms, parm_cov = optimize.curve_fit(model_func, lapsetime01, cut_env01, p0=(np.exp(intersec01), pendiente01, 0))
                AA, KK, CC = opt_parms
                return AA, KK, CC
        
        
            AA, KK, CC = fit_exp_nonlinear(lapsetime01,cut_env01)
            
            fit_y = model_func(lapsetime01, AA, KK, CC)
...
#-----------------------------------------------------------------------------
            ...
            Q01 = (np.pi*fc01*lapsetime01[len(lapsetime01)-1])/b01
            ...
            for n in range(len(lapsetime_vectores)):
                ...
                Qc_prueba = (np.pi*fc_vector[n]*lapsetime_num[len(lapsetime_num)-1])/b_vec[n]
